...
| `v2_design/` (2026-08-31) | The v2 collection sign-off package: measured camera pose (`camera_pose.py`), terminal-approach law vs measured v1 (`terminal_profile.py`), taped-square goal, corrective-episode design (`CONFIRMATION.md`). | Camera: per-scene segmentation pixel counts across the spawn envelope (all PASS). Terminal law: zero-tick parking window as the collection acceptance test. |
